...
export async function seedAdmin(): Promise<void> {
  ...
  while (attempt < maxRetries) {
    try {
      console.log(`Attempting to initialize the database (${attempt + 1}/${maxRetries})...`);
    ...
    await execute("CREATE INDEX IF NOT EXISTS idx_grades_subject ON grades(student_id, subject_id)");
      ...
      attempt++;
      console.warn(`⚠️ The database is not ready yet.: ${error.message}. Waiting for 3 seconds....`);
      
      if (attempt >= maxRetries) {
        console.error('❌ Seeding error after multiple attempts. Check database connection.');
        throw error;
      }
