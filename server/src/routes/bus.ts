...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!direction || !departureTime || !arrivalTime) {
      return res.status(400).json({ error: 'Direction, departure and arrival are mandatory.' });
    }
...
router.put('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Route does not exist.' });
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (count === 0) return res.status(404).json({ error: 'Route does not exist.' });
