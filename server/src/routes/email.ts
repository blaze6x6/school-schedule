...
router.get('/smtp', adminMiddleware, async (_req, res) => {
  try {
    ...
    if (!row) {
      return res.json({ host: '', port: 465, secure: false, user: '', password: '', fromName: 'School Schedule', fromEmail: '' });
    }
    res.json({
      host: row.host,
      port: row.port,
    ...
    res.status(500).json({ error: 'Error retrieving SMTP settings' });
  }
});
...
router.post('/test', adminMiddleware, async (req, res) => {
  try {
    ...
    if (success) {
      ...
      res.status(500).json({ error: 'SMTP is not set up or sending failed' });
    }
  } catch (error) {
