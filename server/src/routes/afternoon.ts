...
router.get('/class/:classId', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error retrieving afternoon activities' });
  }
});
...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    notifyAllWithCalendar(
      'New afternoon activity: ' + name,
      `<p>A new afternoon activity has been added.: <strong>${name}</strong></p>
       <p>${days[dayOfWeek] || ''}, ${startTime} – ${endTime} (every week)</p>
       <p><small>Open the attached .ics file to add to your calendar.</small></p>`,
      { title: name, date: dateStr, startTime, endTime, recurrence: 'weekly', description: name }
    ).catch(() => {});
    notifyAllInApp(`New afternoon activity: ${name} (${days[dayOfWeek]}, ${startTime}–${endTime})`, 'info').catch(() => {});
  } catch (error) {
...
router.put('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Does not exist' });
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (count === 0) return res.status(404).json({ error: 'Does not exist' });
