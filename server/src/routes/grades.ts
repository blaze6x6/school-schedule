...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!studentId || !subjectId || !grade || !type || !date) {
      return res.status(400).json({ error: 'Student, subject, grade, type and date are required.' });
    }
    if (grade < 1 || grade > 5) {
...
router.put('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Rating does not exist' });
...
router.delete('/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    if (count === 0) return res.status(404).json({ error: 'Rating does not exist' });
