...
export default function PeriodsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    const msg = period?.isBreak 
      ...
      : 'Are you sure? This will also delete the schedule entries for this hour.';
    if (confirm(msg)) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">School periods</h1>
          <p className="text-gray-500 text-sm mt-1">Define time ranges for periods and breaks (sorted by time)</p>
        </div>
        <button
          onClick={() => {
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm p-5 mb-4 border-l-4 border-cyan-500">
          <h3 className="font-semibold text-gray-800 mb-3">New period / Break</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Beginning</label>
              <input
                type="time"
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg hover:bg-gray-50">
                ...
                <span className="text-sm text-gray-700">It is a break</span>
              </label>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {!periods || periods.length === 0 ? (
          <div className="p-10 text-center text-gray-500">
            ...
            <p>There are no defined periods.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-5 py-3 text-left text-sm font-semibold text-gray-600">Beginning</th>
                <th className="px-5 py-3 text-left text-sm font-semibold text-gray-600">End</th>
                <th className="px-5 py-3 text-left text-sm font-semibold text-gray-600">Duration</th>
      ...
      <div className="mt-4 p-4 bg-blue-50 rounded-xl">
        <p className="text-sm text-blue-700">
          💡 <strong>Namig:</strong> The periods are automatically sorted by start time. 
          Breaks (e.g. lunch) are displayed in the schedule, but no subjects can be entered for them.
        </p>
      </div>
    </div>
  );
