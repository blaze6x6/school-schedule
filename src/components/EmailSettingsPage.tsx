  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="font-semibold text-gray-800">My email notifications</h2>
          <p className="text-sm text-gray-500">Receive notifications about schedule changes, events, grades and notes</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {enabled ? 'You will receive an email when there are changes.' : 'You will not receive email notifications.'}
              </p>
            </div>
          </div>
          <button
...
function SmtpSettingsForm() {
  const [form, setForm] = useState<SmtpSettings>({
    host: '', port: 465, secure: false, user: '', password: '', fromName: 'School Schedule', fromEmail: '',
  });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setTestResult('❌ ' + (err.message || 'Sending failed'));
    } finally {
      setTesting(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="font-semibold text-gray-800">SMTP server</h2>
          <p className="text-sm text-gray-500">Settings for sending email notifications</p>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Sender's email</label>
          <input
            type="email"
      ...
      <div className="mt-6 pt-5 border-t border-gray-100">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Send a test email</h3>
