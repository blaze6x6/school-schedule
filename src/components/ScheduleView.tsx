...
export default function ScheduleView({ classId, className, title }: Props) {
  ...
  const getEventLabel = (count: number) => {
    ...
    if (count === 3 || count === 4) return 'events';
    return 'events';
  };
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 bg-white rounded-xl p-4 shadow-sm gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-center">
          ...
          <span className="font-semibold text-gray-800">
            {format(weekStart, 'd. MMMM', { locale: enUS })} – {format(addDays(weekStart, 4), 'd. MMMM yyyy', { locale: enUS })}
          </span>
          {!isWeekInSchoolYear && (
            <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-medium rounded-full flex items-center gap-1">
              <Umbrella className="w-3 h-3" /> Holidays
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={showFullName ? 'Show abbreviations' : 'Show full names'}
          >
                    ...
                    {weekDates.map((date, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {!inSchoolYear && (
                            <div className="mt-1 text-[10px] text-gray-400">Holidays</div>
                          )}
