...
export default function ParentsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Parents & Children</h1>

      <div className="bg-white rounded-xl shadow-sm p-5 mb-6 border-l-4 border-indigo-500">
        <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
          <Link className="w-5 h-5 text-indigo-600" /> Connect parent with child
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
          <div>
            <label className="block text-sm text-gray-600 mb-1">Parent (user)</label>
            <select
            ...
            >
              ...
              {users?.map((u: User) => (
                <option key={u.id} value={u.id}>
                  {u.fullName} ({u.username}) — {u.role === 'admin' ? 'Admin' : 'Parent'}
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-1">Child (student)</label>
            <select
              value={selectedChild}
      ...
      <div className="space-y-4">
        {parents.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-10 text-center text-gray-500">
            ...
            <p>There are no parents with related children.</p>
          </div>
        ) : (
          parents.map((parent: User) => (
            <div key={parent.id} className="bg-white rounded-xl shadow-sm p-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {editingParentId === parent.id ? (
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-400">@{parent.username} • {parent.role === 'admin' ? 'Admin + Parent' : 'Parent'}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {parent.childrenIds.length === 0 ? (
                <p className="text-sm text-gray-400 ml-13">There are no related children.</p>
              ) : (
